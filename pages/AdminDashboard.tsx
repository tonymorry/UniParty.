...
const AdminDashboard: React.FC = () => {
  ...
  const activeTab =
    (searchParams.get("tab") as "users" | "events" | "reports" | "archive") ||
    "users";
  ...
  const fetchData = async () => {
    ...
    try {
      if (activeTab === "users") {
        ...
        const data = await api.archive.getAllArchivedListsAdmin();
  ...
  const handleTabChange = (tab: "users" | "events" | "reports" | "archive") => {
    setSearchParams({ tab });
  };
  ...
  const handleDeleteUser = async (userId: string) => {
    if (
      !window.confirm(
        "Sei sicuro di voler eliminare questo utente? Se ha vincoli fiscali verrà solo bloccato (Soft Delete), altrimenti verrà rimosso definitivamente.",
      )
    )
      return;
    try {
  ...
  const downloadArchivedExcel = (list: any) => {
    ...
    link.setAttribute(
      "download",
      `Partecipanti_${list.title.replace(/\s+/g, "_")}.csv`,
    );
  ...
  const filteredEvents = events.filter(
    (e) =>
      ...
      (typeof e.organization === "object" &&
        e.organization.name.toLowerCase().includes(searchTerm.toLowerCase())),
  );

  const getTicketStatusBadge = (ticket: Ticket) => {
    if (ticket.status === "entered") {
      return (
        <span className="px-3 py-1 rounded-full text-xs font-bold bg-yellow-900/30 text-yellow-500 border border-yellow-900/50">
          IN SALA
        </span>
      );
    }
    if (ticket.status === "completed") {
      return (
        <span className="px-3 py-1 rounded-full text-xs font-bold bg-blue-900/30 text-blue-400 border border-blue-900/50">
          COMPLETATO
        </span>
      );
    }
    if (
      ...
      (!ticket.used && !ticket.status)
    ) {
      return (
        <span className="px-3 py-1 rounded-full text-xs font-bold bg-green-900/30 text-green-400 border border-green-900/50">
          VALIDO
        </span>
      );
    }
    ...
    return (
      <span className="px-3 py-1 rounded-full text-xs font-bold bg-gray-700 text-gray-400 border border-gray-600">
        USATO
      </span>
    );
  };
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
          ...
          <div className="bg-gray-800 rounded-lg shadow-lg p-2 flex space-x-2 border border-gray-700">
            ...
            <button
              ...
              className={`px-4 py-2 rounded-md font-medium transition ${activeTab === "archive" ? "bg-indigo-600 text-white" : "text-gray-400 hover:bg-gray-700"}`}
            >
              <div className="flex items-center">
              ...
              <input
                ...
                placeholder={
                  activeTab === "users"
                    ...
                    : "Cerca evento per titolo o organizzatore..."
                }
                  ...
                  <table className="min-w-full divide-y divide-gray-700">
                    ...
                    <tbody className="bg-gray-800 divide-y divide-gray-700">
                      {filteredUsers.map((u) => (
                        <tr
                          key={u._id}
                          className={u.isDeleted ? "bg-red-900/10" : ""}
                        >
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span
                              className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${u.role === UserRole.ASSOCIAZIONE ? "bg-purple-900/30 text-purple-400" : "bg-green-900/30 text-green-400"}`}
                            >
                              {u.role}
                            </span>
                            {u.isVerified && (
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                            ...
                            {u.role === UserRole.ASSOCIAZIONE && (
                              <button
                                ...
                                title={
                                  u.isVerified ? "Revoca Verifica" : "Verifica"
                                }
                              >
                                {u.isVerified ? (
                      ...
                      {filteredEvents.map((e) => (
                        ...
                        >
                          ...
                          <td className="px-6 py-4 whitespace-nowrap flex items-center space-x-2 text-sm">
                            ...
                            <button
                              onClick={() =>
                                navigate(`/events/${e._id}/attendees`)
                              }
                          ...
                          <tr key={r._id}>
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className="flex items-center">
                                <img
                                  src={r.eventId.image}
                                  className="w-8 h-8 rounded object-cover mr-3 border border-gray-700"
                                />
                        ...
                        >
                          <div>
                            ...
                            <div className="flex items-center text-gray-400 text-sm mt-1">
                              ...
                              {list.organization?.name || "Org Sconosciuta"}
                            </div>
                          </div>
                          <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="bg-gray-800 rounded-xl shadow-2xl max-w-2xl w-full max-h-[80vh] flex flex-col border border-gray-700">
            ...
            <div className="p-6 overflow-y-auto flex-1 bg-gray-800">
              {modalLoading ? (
                ...
                <div className="text-center py-8 text-gray-500 italic">
                  Nessun voucher trovato per questo utente.
                </div>
              ) : (
                <div className="space-y-4">
                  {selectedUserTickets.map((ticket) => (
                    <div
                      key={ticket._id}
                      className="border border-gray-700 rounded-lg p-4 flex justify-between items-start bg-gray-900/40 hover:bg-gray-750 transition shadow-sm"
                    >
                      <div>
                        ...
                        <p className="text-sm text-gray-500">
                          {ticket.event.dates && ticket.event.dates.length > 1
                            ...
                            : ticket.event.dates &&
                              ...
                              ? new Date(
                                  ticket.event.dates[0],
                                ).toLocaleDateString()
                              : "N/A"}
                        </p>
                        <p className="text-[10px] text-indigo-400 mt-1 font-mono uppercase tracking-wider">
                          {ticket.qrCodeId}
                        </p>
            ...
            <div className="p-4 border-t border-gray-700 bg-gray-900/50 rounded-b-xl text-right">
              <button
                onClick={() => setIsTicketModalOpen(false)}
                className="px-4 py-2 bg-gray-700 text-gray-300 rounded-lg font-medium hover:bg-gray-600 transition border border-gray-600"
              >
                Chiudi
              </button>
            </div>
          ...
          <div className="bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden border border-gray-700">
            ...
            <div className="p-6 space-y-4">
              <p className="text-gray-300">
                Stai per rimuovere definitivamente l'evento:{" "}
                <strong className="text-red-400 underline">
                  {modTargetEvent.title}
                </strong>
                .
              </p>
              <div>
                <label className="block text-sm font-bold text-gray-400 mb-2 uppercase tracking-wide">
                  Motivazione rimozione (inviata all'associazione)
                </label>
                <textarea
                ...
                <>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    <div className="bg-gray-900 rounded-xl p-4 border border-indigo-900/30 shadow-inner">
                      ...
                      <div className="text-2xl font-bold text-white">
                        €
                        {(
                          selectedEvent.ticketsSold * selectedEvent.price
                        ).toFixed(2)}
                      </div>
                    </div>
                  ...
                  <div className="border-t border-gray-700 pt-6">
                    ...
                    <div className="bg-gray-900 rounded-xl border border-gray-700 overflow-hidden shadow-inner">
                      ...
                      {selectedEventStats &&
                      Object.entries(selectedEventStats).filter(
                        ([k]) => k !== "favorites",
                      ).length > 0 ? (
                        Object.entries(selectedEventStats)
                          ...
                          .map(([name, count]) => (
                            <div
                              key={name}
                              className="grid grid-cols-2 p-3 border-b border-gray-800 last:border-0 hover:bg-gray-800 transition"
                            >
                        ...
                        <div className="p-6 text-center text-gray-600 italic">
                          Nessuna vendita registrata per liste PR.
                        </div>
                      )}
                    </div>
                  </div>
