      ...
      api.events.getById(id).then((data: any) => {
          ...
          if(data) {
              setEditForm({
                  ...
                  times: data.times || (data.dates ? data.dates.map(() => data.time || '22:00') : []),
                  time: data.time,
              ...
              <div className="bg-gray-800 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto border border-gray-700">
                  ...
                  <form onSubmit={handleUpdate} className="p-4 sm:p-6 space-y-4">
                      ...
                      <div className="space-y-4">
                          <label className="block text-sm font-medium text-gray-300 mb-1">Dates & Times</label>
                          <div className="flex flex-col sm:flex-row gap-2 mb-2">
                              ...
                              <button 
                                  type="button" 
                                  onClick={() => {
                                      const dateInput = document.getElementById('newDateInput') as HTMLInputElement;
                                      const timeInput = document.getElementById('newTimeInput') as HTMLInputElement;
                                      if (dateInput.value) {
                                          setEditForm({
                                              ...editForm, 
                                              dates: [...(editForm.dates || []), dateInput.value],
                                              times: [...(editForm.times || []), timeInput.value || '22:00']
                                          });
                                          dateInput.value = '';
                                      }
                                  }}
                          ...
                          <div className="space-y-2">
                              {editForm.dates?.map((d, idx) => (
                                  <div key={idx} className="flex items-center justify-between bg-gray-900/50 p-3 rounded-xl border border-gray-700">
                                      <div className="flex items-center gap-4">
                                          ...
                                          <div className="flex items-center text-sm text-gray-300">
                                              ...
                                              <input 
                                                  type="time"
                                                  value={editForm.times?.[idx] || '22:00'}
                                                  onChange={(e) => {
                                                      const newTimes = [...(editForm.times || [])];
                                                      newTimes[idx] = e.target.value;
                                                      setEditForm({...editForm, times: newTimes});
                                                  }}
                                                  className="bg-transparent border-none focus:ring-0 text-white p-0 w-16"
                                              />
                                          </div>
                                      </div>
                                      <button 
                                          ...
                                          onClick={() => {
                                              ...
                                              const newTimes = editForm.times?.filter((_, i) => i !== idx);
                                              setEditForm({...editForm, dates: newDates, times: newTimes});
                                          }} 
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                           ...
                           <div className="flex items-center space-x-3 bg-gray-900/50 p-3 rounded-lg border border-gray-700">
                               <input 
                                   type="checkbox" 
                                   id="editIsTicketless"
                                   checked={editForm.isTicketless || false}
                                   onChange={e => setEditForm({...editForm, isTicketless: e.target.checked})}
                                   className="h-5 w-5 text-indigo-600 focus:ring-indigo-500 bg-gray-700 border-gray-600 rounded"
                               />
                               <label htmlFor="editIsTicketless" className="text-sm text-gray-300 font-medium cursor-pointer flex items-center">
                ...
                <div className="bg-gray-800 rounded-2xl p-6 shadow-xl border border-gray-700">
                     {event.isTicketless ? (
                         <div className="text-center py-8">
                             ...
                             <p className="text-white font-bold text-lg mb-2">Evento Informativo</p>
                             <p className="text-gray-400 text-sm">
                                 Evento a solo scopo informativo. <br />
