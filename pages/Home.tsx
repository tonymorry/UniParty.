...
const Home: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-white">
      {/* Hero Section */}
      <div className="bg-transparent py-16 md:py-24 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
          <div className="inline-flex items-center px-4 py-1.5 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 text-xs font-black uppercase tracking-[0.2em] mb-6 animate-bounce">
            <Sparkles className="w-3 h-3 mr-2" />
            University Life Reimagined
          </div>
          <h1 className="text-5xl md:text-7xl font-black mb-4 tracking-tighter leading-tight bg-clip-text text-transparent bg-gradient-to-b from-white via-white to-indigo-300">
          ...
          <div className="max-w-2xl mx-auto relative flex items-center gap-3">
            <div className="relative flex-grow">
                <input
                  type="text"
                  placeholder="Search events, locations, associations..."
                  className="w-full pl-6 pr-12 py-5 rounded-2xl bg-white/5 backdrop-blur-md text-white border border-white/10 focus:border-indigo-500/50 focus:ring-4 focus:ring-indigo-500/10 outline-none transition-all placeholder-gray-500 font-medium"
                  value={searchTerm}
            ...
            <button 
                ...
                className={`px-8 py-5 rounded-2xl font-black text-sm uppercase tracking-widest transition-all shadow-xl flex items-center ${showFilters || hasActiveFilters ? 'bg-indigo-600 text-white shadow-indigo-500/40 border border-indigo-400/50' : 'bg-white/5 border border-white/10 text-gray-300 hover:bg-white/10'}`}
            >
      ...
      {!showFilters && (hasActiveFilters || selectedCity !== 'Tutte') && (
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 flex items-center space-x-3 overflow-x-auto">
              <span className="text-[10px] font-black text-gray-600 uppercase tracking-widest mr-2">Active:</span>
              {selectedCity !== 'Tutte' && (
                  <span className="inline-flex items-center px-4 py-1.5 rounded-full text-[11px] font-black bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 uppercase tracking-wider">
              ...
              {selectedTime !== 'all' && (
                  <span className="inline-flex items-center px-4 py-1.5 rounded-full text-[11px] font-black bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 uppercase tracking-wider">
                      {selectedTime}
                      <button onClick={() => setSelectedTime('all')} className="ml-2 hover:text-white transition"><X className="w-3.5 h-3.5"/></button>
                  </span>
              )}
              {selectedCategory !== 'all' && (
                  <span className="inline-flex items-center px-4 py-1.5 rounded-full text-[11px] font-black bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 uppercase tracking-wider">
                      {selectedCategory}
                      <button onClick={() => setSelectedCategory('all')} className="ml-2 hover:text-white transition"><X className="w-3.5 h-3.5"/></button>
                  </span>
              )}
              {selectedPrice !== 'all' && (
                  <span className="inline-flex items-center px-4 py-1.5 rounded-full text-[11px] font-black bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 uppercase tracking-wider">
                      {selectedPrice}
                      <button onClick={() => setSelectedPrice('all')} className="ml-2 hover:text-white transition"><X className="w-3.5 h-3.5"/></button>
                  </span>
              )}
              <button onClick={clearFilters} className="text-[10px] font-black text-gray-600 hover:text-red-400 uppercase tracking-widest ml-auto transition">Clear All</button>
          </div>
      )}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="flex justify-between items-center mb-10">
          <h2 className="text-3xl font-black text-white tracking-tight">
              ...
              <span className="ml-4 text-sm font-bold text-gray-600">({filteredEvents.length})</span>
          </h2>
        </div>
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
             {[1, 2, 3].map(i => (
               <div key={i} className="bg-white/5 border border-white/10 rounded-2xl h-[450px] animate-pulse"></div>
             ))}
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredEvents.length > 0 ? (
              ...
              <div className="col-span-full py-32 text-center glass-card rounded-3xl border-dashed">
                <div className="bg-indigo-500/10 border border-indigo-500/20 rounded-full w-20 h-20 flex items-center justify-center mx-auto mb-6 shadow-[0_0_30px_rgba(99,102,241,0.1)]">
                    <Search className="w-10 h-10 text-indigo-400" />
                </div>
                <h3 className="text-2xl font-black text-white mb-2">No matching events</h3>
                <p className="text-gray-500 font-medium">
                  {selectedCity !== 'Tutte' 
                    ? `Non ci sono eventi al momento a ${selectedCity}.`
                    : 'Try adjusting your filters or search terms.'}
                </p>
                <button onClick={clearFilters} className="mt-8 bg-indigo-600 hover:bg-indigo-700 text-white px-8 py-3 rounded-xl font-bold transition shadow-lg">Clear all filters</button>
              </div>
            )}
          </div>
        )}
