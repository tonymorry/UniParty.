...
const Dashboard: React.FC = () => {
  ...
  // Tab State: 'overview', 'create', 'staff', or 'archive'
  const [activeTab, setActiveTab] = useState<
    "overview" | "create" | "staff" | "archive"
  >("overview");
  ...
  const [scanType, setScanType] = useState<"entry_only" | "entry_exit">(
    "entry_only",
  );
  ...
  const [targetStatus, setTargetStatus] = useState<"active" | "draft">(
    "active",
  );
  ...
  const fetchArchive = async () => {
    ...
    try {
      const data = await api.archive.getMyArchivedLists();
  ...
  const totalTicketsSold = assocEvents.reduce(
    (acc, curr) => acc + curr.ticketsSold,
    0,
  );
  const totalRevenue = assocEvents.reduce(
    (acc, curr) => acc + curr.ticketsSold * curr.price,
    0,
  );

  const handleTabChange = (
    tab: "overview" | "create" | "staff" | "archive",
  ) => {
    setActiveTab(tab);
    ...
    try {
      ...
      if (link) {
        ...
        const confirmed = window.confirm(
          "Simulating Stripe Onboarding. Click OK to finish.",
        );
        if (confirmed) {
  ...
  const handleCreateEvent = async (e: React.FormEvent) => {
    e.preventDefault();
    const cleanPriceStr = (user.stripeOnboardingComplete ? price : "0")
      .toString()
      .replace(",", ".");
  ...
  const handleManageStaff = async (e: React.FormEvent) => {
    ...
    try {
      await api.auth.createStaffAccount({
        email: staffEmail,
        password: staffPassword,
      });
      alert(
        "Account Staff gestito con successo! Fornisci queste credenziali al tuo personale per lo scanner.",
      );
      setStaffEmail("");
      setStaffPassword("");
      fetchStaff();
  ...
  const handleDeleteStaff = async (id: string) => {
    if (!window.confirm("Sei sicuro di voler eliminare questo account staff?"))
      return;
    try {
  ...
  const downloadArchivedExcel = (list: any) => {
    if (!list.attendees || list.attendees.length === 0) return;
    ...
    const rows = list.attendees.map((t: any) => [
      t.ticketHolderName || "",
      t.matricola || "",
      t.emailIstituzionale || "",
      t.prList || "",
      t.corsoStudi || "",
      t.annoCorso || "",
      t.telefono || "",
    ...
    link.setAttribute(
      "download",
      `Lista_${list.title.replace(/\s+/g, "_")}_Archivio.csv`,
    );
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <div className="bg-gray-800 p-1 rounded-xl shadow-sm border border-gray-700 flex flex-wrap justify-center md:justify-start gap-1 md:gap-0">
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-medium transition flex items-center ${activeTab === "archive" ? "bg-indigo-900 text-indigo-100" : "text-gray-400 hover:bg-gray-700"}`}
            >
        ...
        {activeTab === "overview" && (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
            <div
              className={`bg-gray-800 rounded-xl p-6 shadow-sm border-l-4 ${user.stripeOnboardingComplete ? "border-green-500" : "border-blue-500"}`}
            >
            ...
            <div className="bg-gray-800 rounded-2xl shadow-sm p-6 md:p-8 border border-gray-700">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-bold text-white flex items-center">
                  <Briefcase className="w-6 h-6 mr-2 text-indigo-400" />I Tuoi
                  Eventi
                </h2>
              </div>
              ...
              {loadingEvents ? (
                ...
                <div className="divide-y divide-gray-700">
                  {assocEvents.map((event) => (
                    <div
                      key={event._id}
                      className="py-4 flex flex-col md:flex-row md:items-center justify-between group gap-4"
                    >
                ...
                <div className="text-center py-12 bg-gray-900 rounded-xl border border-dashed border-gray-700">
                  ...
                  <button
                    onClick={() => handleTabChange("create")}
                    className="bg-gray-800 border border-gray-700 px-4 py-2 rounded-lg text-sm font-medium hover:bg-gray-700 text-white"
                  >
                    Inizia ora
                  </button>
                </div>
            ...
            {!user.isVerified ? (
              ...
              <div className="bg-gray-800 rounded-xl shadow-lg overflow-hidden border border-gray-700">
                ...
                >
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-300 mb-1 flex items-center">
                        <Tag className="w-4 h-4 mr-1 text-indigo-400" />{" "}
                        Categoria
                      </label>
                      <select
                        value={category}
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-300 mb-1 flex items-center">
                      <FileText className="w-4 h-4 mr-1 text-indigo-400" />{" "}
                      Descrizione Evento
                    </label>
                    <textarea
                      value={description}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                      <label className="block text-sm font-medium text-gray-300 mb-1 flex items-center">
                        <Clock className="w-4 h-4 mr-1 text-indigo-400" /> Date
                        dell'Evento
                      </label>
                    ...
                    <div>
                      ...
                      >
                        <option value="">
                          {selectedRegion
                            ? "Seleziona Città..."
                            : "Scegli prima una regione"}
                        </option>
                        {selectedRegion &&
                          UNIVERSITY_LOCATIONS[selectedRegion].map((c) => (
                            <option key={c} value={c}>
                              {c}
                            </option>
                          ))}
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-300 mb-1 flex items-center">
                      <ImageIcon className="w-4 h-4 mr-1 text-indigo-400" />{" "}
                      Immagine Evento
                    </label>
                  ...
                  <div className="p-4 bg-gray-900/30 rounded-xl border border-gray-700">
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 mb-3">
                      <input
                        ...
                        onKeyPress={(e) =>
                          e.key === "Enter" &&
                          (e.preventDefault(), handleAddPrList())
                        }
                    ...
                    <div className="flex flex-wrap gap-2">
                      {prLists.map((list) => (
                        <span
                          key={list}
                          className="bg-indigo-900/40 text-indigo-100 px-3 py-1 rounded-full text-sm flex items-center border border-indigo-900/50"
                        >
                          {list}
                          <button
                            type="button"
                            onClick={() => handleRemovePrList(list)}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-4 bg-gray-900/30 rounded-xl border border-gray-700">
                    <div className="flex items-center space-x-3">
                      <input
                        type="checkbox"
                        id="isTicketless"
                        checked={isTicketless}
                        onChange={(e) => setIsTicketless(e.target.checked)}
                        className="h-5 w-5 text-indigo-600 focus:ring-indigo-500 bg-gray-700 border-gray-600 rounded"
                      />
                      <label
                        htmlFor="isTicketless"
                        className="text-sm text-gray-300 font-medium cursor-pointer flex items-center"
                      >
                        <Info className="w-4 h-4 mr-2 text-indigo-400" />
                        Evento puramente informativo (nessun biglietto)
                      </label>
                    </div>
                    <div className="flex items-center space-x-3">
                      <input
                        ...
                        onChange={(e) =>
                          setRequiresAcademicData(e.target.checked)
                        }
                        className="h-5 w-5 text-indigo-600 focus:ring-indigo-500 bg-gray-700 border-gray-600 rounded"
                      />
                      <label
                        htmlFor="reqAcademicData"
                        className="text-sm text-gray-300 font-medium cursor-pointer flex items-center"
                      >
                    ...
                    <div>
                      ...
                      <select
                        ...
                        className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none text-white"
                      >
                        <option value="entry_only">
                          Solo Entrata (Classic)
                        </option>
                        <option value="entry_exit">
                          Entrata + Uscita (Monitoraggio Tempo)
                        </option>
                      </select>
                    </div>
                  </div>
          ...
          <div className="animate-in fade-in slide-in-from-bottom-2 duration-300 space-y-6">
            <div className="bg-gray-800 rounded-xl shadow-lg border border-gray-700 overflow-hidden">
              <div className="bg-indigo-900/30 p-6 text-white border-b border-gray-700">
                ...
                <p className="text-gray-400 text-sm mt-1">
                  Crea o aggiorna un account delegato per scansionare i voucher
                  all'ingresso.
                </p>
              </div>

              <form
                onSubmit={handleManageStaff}
                ...
                <div className="bg-amber-900/10 border-l-4 border-amber-500 p-4 rounded-r-lg mb-6">
                  <div className="flex">
                    ...
                    <p className="text-sm text-amber-200">
                      Questi account hanno accesso{" "}
                      <strong>solo allo scanner</strong>. Possono validare
                      voucher esclusivamente per i tuoi eventi.
                    </p>
                  </div>
                </div>
            ...
            <div className="bg-gray-800 rounded-xl shadow-lg border border-gray-700 overflow-hidden">
              <div className="bg-indigo-900/30 p-6 text-white border-b border-gray-700">
                <h2 className="text-xl font-bold flex items-center text-indigo-400">
                  ...
                  Archivio Liste Presenze
                </h2>
                <p className="text-gray-400 text-sm mt-1">
                  Qui trovi le liste dei partecipanti degli eventi gratuiti che
                ...
                {loadingArchive ? (
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    {archivedLists.map((list) => (
                      <div
                        key={list._id}
                        className="bg-gray-900/50 p-4 rounded-xl border border-gray-700 flex flex-col md:flex-row md:items-center justify-between gap-4 hover:bg-gray-700 transition"
                      >
                        <div>
                          ...
                          <div className="flex items-center text-gray-400 text-sm mt-1">
                            ...
                            {list.attendees?.length || 0} Partecipanti
                          </div>
                        </div>
                        <button
                          onClick={() => downloadArchivedExcel(list)}
                          className="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg font-bold transition flex items-center justify-center shadow-md"
                        >
                  ...
                  <div className="text-center py-12 bg-gray-900 rounded-xl border border-dashed border-gray-700 text-gray-500">
                    Nessuna lista archiviata trovata.
                  </div>
                )}
              </div>
            </div>
